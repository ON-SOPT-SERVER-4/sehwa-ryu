...
router.post('/signup', (req, res) => {
  ...
  const idCheck = usersDB.find(user => user.id == id);
  if (idCheck) {
...
router.post('/signin', (req, res) => {
  ...
  const idCheck2 = usersDB.filter(user => user.id == id);
  if (idCheck2.length == 0) {
  ...
  const digest = idCheck2[0].salt;
  const digestPw = idCheck2[0].password;
  crypto.pbkdf2(password, digest, 100000, 64, 'sha512', (err, key) => {
    if (key.toString('base64') === digestPw) {
      console.log('로그인 성공!');
